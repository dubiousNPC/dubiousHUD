...
------------------------------ constants ------------------------------
local SELECT_RENDERER_ID = "SuperSelect3"

-- session only install flags for other mods, "SuperSelect3" = true, "SuperSelect" = 3
...
installedRenderers:set(SELECT_RENDERER_ID, true)
local familyKey, familyVersion = SELECT_RENDERER_ID:match("^(.-)(%d+)$")
...
local function extraButton(spec, rowHeight, textSize, value)
	...
	-- bordered content carries no events, so update() stays a plain refresh
	-- both branches match the padded select row height, so the button lines up beside it
	local bordered
	if spec.icon then
		...
		-- not, collapsing the button); square at rowHeight + border stands in for the
		-- padding wrapper the select row has, matching its height without one
		...
		bordered = { template = I.MWUI.templates.box, content = ui.content{ imageElement } }
	else
		-- fixed height in a paddedBox reproduces the select's box + padding + rowHeight
		-- stack, so text buttons are the same height as the row (autoSize was ~4px shorter,
		-- missing the padding); width honors spec.width like the docs promise
		textElement = {
			template = I.MWUI.templates.textNormal,
			props = {
				text = spec.text or '',
				...
				textAlignV = ui.ALIGNMENT.Center,
				autoSize = false,
				size = v2(spec.width or 60, rowHeight),
			},
		}
		bordered = paddedBox(textElement)
	end
...
-- a plain Widget never takes key focus, so escape lands here
local function onKeyPress(key)
	if key.code == input.KEY.Escape then closeDropdown() end
end
