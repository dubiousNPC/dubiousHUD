...
function borderTemplates(thickness,color,borderSize,background,padding)
	...
	-- slot, inset by border so injected content clears the edge
	result.borders.content:add {
		external = { slot = true },
		props = {
			relativeSize = v2(1, 1),
	...
	-- nestable padding template, grows by 2*pad around its slot
	result.padding = {
