---@omw-context menu
local ui = require('openmw.ui')
local util = require('openmw.util')
local async = require('openmw.async')
local I = require('openmw.interfaces')
local auxUi = require('openmw_aux.ui')
local input = require('openmw.input')
local storage = require('openmw.storage')

-- Example:
-- {
-- 	key = "OUTLINE_COLOR",
-- 	name = "Outline color",
-- 	description = "",
-- 	renderer = "SuperColorPicker4",
-- 	default = util.color.hex("ffcc66"),
-- 	argument = { -- optional, nil is fine
-- 		presetColors = { "ffcc66", "202020" }, -- default: the shared morrowind palette, hex strings, shown as preset swatches, {} for none
-- 		disabled = false, -- default: false
-- 		historyKey = "MyMod", -- default: nil, own recent colors instead of the shared list
-- 	},
-- },
--
-- stored values: util.color, same as the engine "color" renderer
-- the picker window is draggable by its background and remembers its position across opens
-- recent colors are shared across mods, right click a swatch to drop one

local M = {}
local boxOffset = 25
local displayArea = ui.layers[1].size
local boxWidth = 600
local boxHeight = 400

-- =========================================================
-- =========================================================
local COLOR_RENDERER_ID = "SuperColorPicker4"

-- session only install flags for other mods, "SuperColorPicker4" = true, "SuperColorPicker" = 4
local installedRenderers = storage.playerSection("InstalledSettingsRenderers")
installedRenderers:setLifeTime(storage.LIFE_TIME.GameSession)
installedRenderers:set(COLOR_RENDERER_ID, true)
local familyKey, familyVersion = COLOR_RENDERER_ID:match("^(.-)(%d+)$")
if (installedRenderers:get(familyKey) or 0) < tonumber(familyVersion) then
	installedRenderers:set(familyKey, tonumber(familyVersion))
end

-- COLOR HISTORY CONFIGURATION
local MAX_HISTORY_COLORS = 40
local SWATCH_SIZE = 22
local SWATCH_SPACING = 4

-- shown when a setting passes no presetColors, pass {} to opt out
local defaultPresetColors = {
	"caa560", -- fontColor_color_normal
	"d4b77f", -- goldenMix
	"dfc99f", -- FontColor_color_normal_over
	"eee2c9", -- lightText
	"253170", -- fontColor_color_journal_link
	"3a4daf", -- fontColor_color_journal_link_over
	"6070ca", -- FontColor_color_active
	"707ecf", -- fontColor_color_journal_link_pressed
	"c83c1e", -- FontColor_color_health
	"35459f", -- FontColor_color_magic
	"00963c", -- FontColor_color_fatigue
}

local colorBoxElement = nil

-- Module-level picker state for mousewheel access
local scrollButton = nil
local crossHair = nil
local newColor = nil
local pickerSet = nil
local pickerUpdateUI = nil

-- =========================================================
-- Color History Management
-- =========================================================
-- family keyed, so a version bump keeps the palette
local storageKey = familyKey or COLOR_RENDERER_ID
local sharedHistorySection = storage.playerSection(storageKey.."History")

-- persistent window position, absolute top-left on the Settings layer, so the picker reopens where you left it
local windowPosSection = storage.playerSection(storageKey.."WindowPos")

local function historySectionFor(argument)
	local historyKey = argument and argument.historyKey
	if not historyKey then return sharedHistorySection end
	return storage.playerSection(storageKey.."History_"..historyKey)
end

-- keeps a 40px grabbable strip on screen, top edge never leaves the top
local function clampWindowPos(pos)
	local screen = ui.layers[ui.layers.indexOf('Settings')].size
	return util.vector2(
		util.clamp(pos.x, 40 - boxWidth, screen.x - 40),
		util.clamp(pos.y, 0, screen.y - 40)
	)
end




local function loadColorHistory(section)
	local historyData = section:get("colors")
	local colors = {}
	
	if historyData then
		for i, hexStr in ipairs(historyData) do
			table.insert(colors, util.color.hex(hexStr))
		end
	end
	
	return colors
end

local function hexKey(hexStr)
	return (hexStr:gsub("^#", "")):upper()
end

local function presetSlotCount(presetColors)
	return math.min(#presetColors, MAX_HISTORY_COLORS)
end

-- display list, storage still holds every picked color including preset matches
local function loadColorHistoryForUi(section, presetColors)
	presetColors = presetColors or {}
	local historyData = section:get("colors") or {}
	local historySlots = MAX_HISTORY_COLORS - presetSlotCount(presetColors)
	local colors = {}
	
	local seen = {}
	for _, hexStr in ipairs(presetColors) do
		seen[hexKey(hexStr)] = true
	end
	
	for _, hexStr in ipairs(historyData) do
		if #colors >= historySlots then break end
		local key = hexKey(hexStr)
		if not seen[key] then
			seen[key] = true
			table.insert(colors, util.color.hex(hexStr))
		end
	end
	
	for i = 1, presetSlotCount(presetColors) do
		colors[historySlots + i] = util.color.hex(presetColors[i])
	end
	return colors
end



local function saveColorHistory(section, colors)
	local hexStrings = {}
	for i, col in ipairs(colors) do
		table.insert(hexStrings, col:asHex())
	end
	section:set("colors", hexStrings)
end

local function addColorToHistory(section, color)
	local history = loadColorHistory(section)
	local colorHex = color:asHex():upper()
	
	-- remove any existing copy
	for i = #history, 1, -1 do
		if history[i]:asHex():upper() == colorHex then
			table.remove(history, i)
			break
		end
	end
	
	table.insert(history, 1, color)
	
	while #history > MAX_HISTORY_COLORS do
		table.remove(history)
	end
	
	saveColorHistory(section, history)
	return history
end

local function removeColorFromHistory(section, color)
	local history = loadColorHistory(section)
	local colorHex = color:asHex():upper()
	
	for i = #history, 1, -1 do
		if history[i]:asHex():upper() == colorHex then
			table.remove(history, i)
		end
	end
	
	saveColorHistory(section, history)
end

-- =========================================================
-- Helper Functions
-- =========================================================

local function hsvToRgb(h, s, v)
	s = s / 100
	v = v / 100
	
	local c = v * s
	local x = c * (1 - math.abs((h / 60) % 2 - 1))
	local m = v - c
	
	local r1, g1, b1
	
	if h < 60 then r1,g1,b1 = c,x,0
	elseif h < 120 then r1,g1,b1 = x,c,0
	elseif h < 180 then r1,g1,b1 = 0,c,x
	elseif h < 240 then r1,g1,b1 = 0,x,c
	elseif h < 300 then r1,g1,b1 = x,0,c
	else r1,g1,b1 = c,0,x
	end
	
	return
		math.floor((r1+m)*255),
		math.floor((g1+m)*255),
		math.floor((b1+m)*255)
end

local function rgbToHsv(r, g, b)
	-- Use floats in [0,1] for exact inversion
	r, g, b = r / 255, g / 255, b / 255
	
	local max = math.max(r, g, b)
	local min = math.min(r, g, b)
	local delta = max - min
	
	local h = 0
	if delta == 0 then
		h = 0
	elseif max == r then
		h = 60 * (( (g - b) / delta) % 6)
	elseif max == g then
		h = 60 * (((b - r) / delta) + 2)
	elseif max == b then
		h = 60 * (((r - g) / delta) + 4)
	end
	
	local s = 0
	if max ~= 0 then
		s = delta / max
	end
	
	local v = max
	
	-- scale S and V to 0-100 to match hsvToRgb
	s = s * 100
	v = v * 100
	
	return h, s, v
end

-- unmaps a color to x, y, z positions
local function unmapColorToPositions(color)
	-- accept either util.color userdata or table {r,g,b}
	local r,g,b = util.round(color.r*255), util.round(color.g*255), util.round(color.b*255)
	
	-- Convert RGB to HSV
	local h, s, v = rgbToHsv(r, g, b)
	
	-- Remap HSV back to coordinates
	local x = util.round(util.remap(h, 0, 360, 0, 256))
	local y = util.round(util.remap(s, 100, 0, 0, 256))  -- notice S was inverted
	local z = util.round(util.remap(v, 100, 0, 0, 236))  -- V was ScrollButton.y
	
	return x, y, z
end

local function resetState(element, s)
	if element then
		element:destroy()
		element = nil
	end
	if s then
		s.confirmed = false
		s.capturedButton = nil
		s.capturedKey = nil
	end
end

local function transparentBorders(layout)
	for _, child in ipairs(layout.content or {}) do
		if child.template and child.template.props then
			child.template.props.alpha = 0
		end
	end
end

-- Create invisible border by copying thin box and hiding all border images
local invisibleBox = auxUi.deepLayoutCopy(I.MWUI.templates.box)
transparentBorders(invisibleBox)

local function makeLabelBox(name, text)
	return {
		template = invisibleBox,
		--type = ui.TYPE.Container,
		name = name .. "LabelBox",
		props = {},
		content = ui.content{
			{
				template = I.MWUI.templates.padding,
				name = name .. "LabelPad",
				props = {},
				content = ui.content{
					{
						template = I.MWUI.templates.textNormal,
						name = name .. "Label",
						props = {
							text = text,
							textAlignH = ui.ALIGNMENT.Start,
							textAlignV = ui.ALIGNMENT.Center,
						},
						content = ui.content{},
					},
				},
			},
		},
	}
end

--- Creates colorPicker msg box
local function makeColorPickerBox(value, set, argument)
	local presetColors = argument and argument.presetColors or defaultPresetColors
	local historySection = historySectionFor(argument)
	-- ==== CONSTANTS & INITIAL COLOR ====
	local rsvMap      = ui.texture{ path = 'textures/HSV.dds' }
	local crossHairTex = ui.texture{ path = 'textures/crosshair007.dds' }
	local btnHeight   = 20
	local rr = util.round(value.r * 255)
	local gg = util.round(value.g * 255)
	local bb = util.round(value.b * 255)
	local T_rgb = { rr, gg, bb }
	
	-- reverted to on cancel and escape
	local initialColor = value
	
	-- Store set function for mousewheel access
	pickerSet = set
	
	-- current working color (module-level for mousewheel access)
	newColor = value
	
	-- text input buffers
	local lastRGBInput = nil
	local lastHexInput = nil
	
	-- widget references (scrollButton and crossHair are module-level)
	local hexInput, hexText, rgbText, displayColorBox, rsvMapImage
	local historySwatches = {}  -- References to history swatch widgets
	
	-- simple registry for RGB inputs
	local inputs = {}
	
	-- initial picker positions from starting color
	local x, y, h = unmapColorToPositions(value)
	
	-- initial palette darkness based on value
	local initialV = util.remap(h, 0, 236, 100, 0) / 100
	
	----------------------------------------------------------------
	-- HELPERS (use shared locals above)
	----------------------------------------------------------------
	
	local loadColorFromHistory
	
	-- update hex/R/G/B displays + text labels + preview color box
	local function updateUIFromColor()
		local c = newColor or value
		local r = util.round(c.r * 255)
		local g = util.round(c.g * 255)
		local b = util.round(c.b * 255)
		local hex = c:asHex():upper()
		
		if hexInput then
			hexInput.props.text = hex
		end
		
		if inputs.R then inputs.R.props.text = tostring(r) end
		if inputs.G then inputs.G.props.text = tostring(g) end
		if inputs.B then inputs.B.props.text = tostring(b) end
		
		if hexText then
			hexText.props.text = "Hex: ##" .. hex
		end
		
		if rgbText then
			rgbText.props.text = string.format("R:%d G: %d B:%d", r, g, b)
		end
		
		if displayColorBox and displayColorBox.content and displayColorBox.content[1] then
			displayColorBox.content[1].props.color = c
		end
		
		-- Darken the HSV palette based on the value slider position
		if rsvMapImage and scrollButton then
			local v = util.remap(scrollButton.props.position.y, 0, 236, 100, 0) / 100
			rsvMapImage.props.color = util.color.rgb(v, v, v)
		end
		
		if colorBoxElement then
			colorBoxElement:update()
		end
	end
	
	-- Store for mousewheel access
	pickerUpdateUI = updateUIFromColor
	
	
	loadColorFromHistory = function(color)
		newColor = color
		set(newColor)
		
		local px, py, ph = unmapColorToPositions(newColor)
		if crossHair then
			crossHair.props.position = util.vector2(px, py)
		end
		if scrollButton then
			scrollButton.props.position = util.vector2(266, ph)
		end
		
		updateUIFromColor()
	end
	
	local function onRGBTextChanged(text)
		lastRGBInput = text
	end
	
	local function makeOnRGBFocusLoss(component)  -- "R", "G", or "B"
		return function()
			if not lastRGBInput then return end
			local ok, parsed = pcall(function()
				return tonumber(lastRGBInput)
			end)
			
			local baseColor = newColor or value
			
			-- invalid input -> restore old value
			if not ok or not parsed then
				local key = string.lower(component) -- "r", "g", "b"
				local fallback = util.round(baseColor[key] * 255)
				if inputs[component] then
					inputs[component].props.text = tostring(fallback)
				end
				lastRGBInput = nil
				if colorBoxElement then colorBoxElement:update() end
				return
			end
			
			parsed = util.round(util.clamp(parsed, 0, 255))
			
			-- update just this component
			local r = baseColor.r
			local g = baseColor.g
			local b = baseColor.b
			
			if component == "R" then
				r = parsed / 255
			elseif component == "G" then
				g = parsed / 255
			elseif component == "B" then
				b = parsed / 255
			end
			
			newColor = util.color.rgb(r, g, b)
			set(newColor)  -- Live update to storage
			
			-- update text boxes + labels + preview
			if inputs[component] then
				inputs[component].props.text = tostring(parsed)
			end
			
			-- reposition crosshair + scrollButton based on new color
			local px, py, ph = unmapColorToPositions(newColor)
			if crossHair then
				crossHair.props.position = util.vector2(px, py)
			end
			if scrollButton then
				scrollButton.props.position = util.vector2(266, ph)
			end
			
			updateUIFromColor()
			lastRGBInput = nil
		end
	end
	
	local function onHexTextChanged(text)
		lastHexInput = text
	end
	
	local function onHexFocusLoss()
		if not lastHexInput then return end
		-- Try parsing exactly what the user typed
		local ok, parsedColor = pcall(util.color.hex, lastHexInput)
		
		if not ok or not parsedColor then
			-- Revert to existing color
			local c = newColor or value
			if hexInput then
				hexInput.props.text = c:asHex()  -- lowercase, no '#'
			end
			lastHexInput = nil
			if colorBoxElement then colorBoxElement:update() end
			return
		end
		-- Apply new color
		newColor = parsedColor
		set(newColor)  -- Live update to storage
		
		-- Update UI handles
		local px, py, ph = unmapColorToPositions(newColor)
		
		if crossHair then
			crossHair.props.position = util.vector2(px, py)
		end
		if scrollButton then
			scrollButton.props.position = util.vector2(266, ph)
		end
		
		updateUIFromColor()
		lastHexInput = nil
	end
	
	----------------------------------------------------------------
	-- INPUT BOX CONSTRUCTOR
	----------------------------------------------------------------
	local function makeInputBox(i, letter, currColor)
		local upper = string.upper(letter)
		local inputWidget = {
			template = I.MWUI.templates.textEditLine,
			name = upper,
			props = {
				text = tostring(currColor),
				size = util.vector2(65, 0),
				textAlignH = ui.ALIGNMENT.Center,
				textAlignV = ui.ALIGNMENT.Center,
			},
			content = ui.content{},
		}
		inputWidget.events = {
			textChanged = async:callback(onRGBTextChanged),
			focusLoss   = async:callback(makeOnRGBFocusLoss(upper)),
		}
		local box = {
			template = I.MWUI.templates.box,
			name = upper .. "Box",
			props = {
				position = util.vector2(45, 60 + boxOffset * i),
				relativePosition = util.vector2(0.55, 0.05),
			},
			content = ui.content{
				{
					template = I.MWUI.templates.padding,
					name = upper .. "Pad",
					content = ui.content{ inputWidget },
				}
			}
		}
		return inputWidget, box
	end
	
	----------------------------------------------------------------
	-- BLOCKER
	----------------------------------------------------------------
	local blocker = {
		name = 'Blocker',
		type = ui.TYPE.Widget,
		props = {
			size = displayArea,
		},
		content = ui.content{},
		events = {
			mouseClick = async:callback(function()
				set(initialColor)  -- Revert to initial color
				if colorBoxElement then
					colorBoxElement:destroy()
					colorBoxElement = nil
				end
				newColor = nil
			end),
		},
	}
	
	----------------------------------------------------------------
	-- OK / CANCEL BUTTONS
	----------------------------------------------------------------
	local okBtn = {
		template = I.MWUI.templates.boxSolid,
		name = nil,
		props = {
			size = util.vector2(120, 40),
			position = nil,
			relativePosition = util.vector2(0.75, 0.98),
			anchor = util.vector2(0.5, 1),
		},
		content = ui.content{
			{
				template = I.MWUI.templates.padding,
				props = {},
				content = ui.content{
					{
						template = I.MWUI.templates.textNormal,
						props = {
							text = "  OK  ",
							size = util.vector2(120, 40),
							textAlignH = ui.ALIGNMENT.Center,
							textAlignV = ui.ALIGNMENT.Center,
						},
						content = ui.content{},
					},
				},
			},
		},
		events = {
			mouseClick = async:callback(function()
				-- Color is already saved via live updates, just close
				if newColor then
					set(newColor)  -- Ensure final value is set
					-- Add to color history
					addColorToHistory(historySection, newColor)
				end
				resetState(colorBoxElement)
				colorBoxElement = nil
			end),
		},
	}
	
	local cancelBtn = {
		template = I.MWUI.templates.boxSolid,
		name = nil,
		props = {
			size = util.vector2(120, 40),
			position = nil,
			relativePosition = util.vector2(0.25, 0.98),
			anchor = util.vector2(0.5, 1),
		},
		content = ui.content{
			{
				template = I.MWUI.templates.padding,
				props = {},
				content = ui.content{
					{
						template = I.MWUI.templates.textNormal,
						props = {
							text = "Cancel",
							size = util.vector2(120, 40),
							textAlignH = ui.ALIGNMENT.Center,
							textAlignV = ui.ALIGNMENT.Center,
						},
						content = ui.content{},
					},
				},
			},
		},
		events = {
			mouseClick = async:callback(function()
				set(initialColor)  -- Revert to initial color
				resetState(colorBoxElement)
				colorBoxElement = nil
				newColor = nil
			end),
		},
	}
	
	----------------------------------------------------------------
	-- HSV MAP IMAGE
	----------------------------------------------------------------
	rsvMapImage = {
		type = ui.TYPE.Image,
		name = 'rsvMap',
		props = {
			size = util.vector2(256, 256),
			position = util.vector2(0, 0),
			relativePosition = util.vector2(0.05, 0.05),
			anchor = nil,
			visible = true,
			alpha = 1.0,
			inheritAlpha = false,
			resource = rsvMap,
			color = util.color.rgb(initialV, initialV, initialV),
		},
		content = ui.content{},
		userData = {
			doDrag = false,
			lastMousePos = nil,
			truePosition = nil,
		},
		events = {
			mousePress = async:callback(function(coord, layout)
				layout.userData.lastMousePos = coord.offset
				if layout.userData.doDrag then return end
				
				local mx = coord.offset.x
				local my = coord.offset.y
				
				if mx < 0 then mx = 0 elseif mx > 256 then mx = 256 end
				if my < 0 then my = 0 elseif my > 256 then my = 256 end
				
				if crossHair then
					crossHair.props.position = util.vector2(mx, my)
				end
				
				local h = util.round(util.remap(mx, 0, 256, 0, 360))
				local s = util.round(util.remap(my, 0, 256, 100, 0))
				local v = scrollButton and scrollButton.props.position.y or h
				v = util.round(util.remap(v, 0, 236, 100, 0))
				
				local r, g, b = hsvToRgb(h, s, v)
				newColor = util.color.rgb(r / 255, g / 255, b / 255)
				set(newColor)  -- Live update to storage
				
				updateUIFromColor()
				layout.userData.doDrag = true
			end),
			
			mouseRelease = async:callback(function(_, layout)
				layout.userData.doDrag = false
			end),
			
			mouseMove = async:callback(function(coord, layout)
				if not layout.userData.doDrag then return end
				
				local mx = coord.offset.x
				local my = coord.offset.y
				
				if mx < 0 then mx = 0 elseif mx > 256 then mx = 256 end
				if my < 0 then my = 0 elseif my > 256 then my = 256 end
				
				if crossHair then
					crossHair.props.position = util.vector2(mx, my)
				end
				
				layout.userData.lastMousePos = coord.offset
				
				local h = util.round(util.remap(mx, 0, 256, 0, 360))
				local s = util.round(util.remap(my, 0, 256, 100, 0))
				local v = scrollButton and scrollButton.props.position.y or h
				v = util.round(util.remap(v, 0, 236, 100, 0))
				
				local r, g, b = hsvToRgb(h, s, v)
				newColor = util.color.rgb(r / 255, g / 255, b / 255)
				set(newColor)  -- Live update to storage
				
				updateUIFromColor()
			end),
		},
	}
	
	----------------------------------------------------------------
	-- CROSSHAIR
	----------------------------------------------------------------
	crossHair = {
		type = ui.TYPE.Image,
		name = "crossHair",
		props = {
			size = util.vector2(20, 20),
			position = util.vector2(x, y),
			relativePosition = util.vector2(0.05, 0.05),
			anchor = util.vector2(0.5, 0.5),
			visible = true,
			alpha = 1.0,
			inheritAlpha = false,
			resource = crossHairTex,
			color = util.color.rgb(0, 0, 0),
		},
		content = ui.content{},
		userData = {
			doDrag = false,
			lastMousePos = nil,
			truePosition = nil,
			oneShot = nil,
		},
		events = {
			mousePress = async:callback(function(coord, layout)
				layout.userData.doDrag = true
				layout.userData.lastMousePos = coord.position
				layout.userData.oneShot = nil
			end),
			
			mouseRelease = async:callback(function(_, layout)
				layout.userData.doDrag = false
				layout.userData.oneShot = nil
			end),
			
			mouseMove = async:callback(function(coord, layout)
				if not layout.userData.doDrag then return end
				
				local props = layout.props
				local dxy = coord.position - layout.userData.lastMousePos
				
				if not layout.userData.oneShot then -- Required to init truePosition correctly
					layout.userData.oneShot = true
					layout.userData.truePosition = props.position
				end
				
				layout.userData.truePosition = layout.userData.truePosition + dxy
				
				local mx = layout.userData.truePosition.x
				local my = layout.userData.truePosition.y
				
				if mx < 0 then mx = 0 elseif mx > 256 then mx = 256 end
				if my < 0 then my = 0 elseif my > 256 then my = 256 end
				
				props.position = util.vector2(mx, my)
				layout.userData.lastMousePos = coord.position
				
				local h = util.round(util.remap(mx, 0, 256, 0, 360))
				local s = util.round(util.remap(my, 0, 256, 100, 0))
				local v = scrollButton and scrollButton.props.position.y or h
				v = util.round(util.remap(v, 0, 236, 100, 0))
				
				local r, g, b = hsvToRgb(h, s, v)
				newColor = util.color.rgb(r / 255, g / 255, b / 255)
				set(newColor)  -- Live update to storage
				
				updateUIFromColor()
			end),
		},
	}
	
	----------------------------------------------------------------
	-- SCROLL TRACK & BUTTON
	----------------------------------------------------------------
	local scrollTrack = {
		type = ui.TYPE.Image,
		name = "ScrollTrack",
		props = {
			size = util.vector2(10, 256),
			position = util.vector2(266, 0),
			relativePosition = util.vector2(0.05, 0.05),
			anchor = util.vector2(0, 0),
			visible = true,
			alpha = 1.0,
			inheritAlpha = false,
			resource = ui.texture{ path = 'white' },
			color = util.color.rgb(52/255, 52/255, 52/255),
		},
		content = ui.content{},
	}
	
	scrollButton = {
		type = ui.TYPE.Image,
		name = "ScrollButton",
		props = {
			size = util.vector2(10, btnHeight),
			position = util.vector2(266, h),
			relativePosition = util.vector2(0.05, 0.05),
			anchor = util.vector2(0, 0),
			visible = true,
			alpha = 1.0,
			inheritAlpha = false,
			resource = ui.texture{ path = 'white' },
			color = util.color.rgb(203/255, 182/255, 140/255),
		},
		content = ui.content{},
		userData = {
			doDrag = false,
			lastMousePos = nil,
		},
		events = {
			mousePress = async:callback(function(coord, layout)
				layout.userData.doDrag = true
				layout.userData.lastMousePos = coord.position
			end),
			
			mouseRelease = async:callback(function(_, layout)
				layout.userData.doDrag = false
			end),
			
			mouseMove = async:callback(function(coord, layout)
				if not layout.userData.doDrag then return end
				
				local props = layout.props
				local dy = coord.position.y - layout.userData.lastMousePos.y
				props.position = props.position + util.vector2(0, dy)
				props.position = util.vector2(props.position.x, util.clamp(props.position.y, 0, 236))
				layout.userData.lastMousePos = coord.position
				
				local mx = crossHair and crossHair.props.position.x or 0
				local my = crossHair and crossHair.props.position.y or 0
				
				local h = util.round(util.remap(mx, 0, 256, 0, 360))
				local s = util.round(util.remap(my, 0, 256, 100, 0))
				local v = props.position.y
				v = util.round(util.remap(v, 0, 236, 100, 0))
				
				local r, g, b = hsvToRgb(h, s, v)
				newColor = util.color.rgb(r / 255, g / 255, b / 255)
				set(newColor)  -- Live update to storage
				
				updateUIFromColor()
			end),
		},
	}
	
	----------------------------------------------------------------
	-- DISPLAY COLOR BOX
	----------------------------------------------------------------
	displayColorBox = {
		template = I.MWUI.templates.boxThick,
		name = "displayColorBox",
		props = {
			size = util.vector2(30, 30),
			position = util.vector2(0, 0),
			relativePosition = util.vector2(0.55, 0.05),
			anchor = util.vector2(0, 0),
		},
		content = ui.content{
			{
				type = ui.TYPE.Image,
				name = "displayColor",
				props = {
					size = util.vector2(30, 30),
					position = util.vector2(0, 0),
					relativePosition = util.vector2(0.55, 0.05),
					anchor = util.vector2(0, 0),
					visible = true,
					alpha = 1.0,
					inheritAlpha = false,
					resource = ui.texture{ path = 'white' },
					color = value,
				},
				content = ui.content{},
			},
		},
	}
	
	----------------------------------------------------------------
	-- HEX / RGB TEXT LABELS
	----------------------------------------------------------------
	hexText = {
		template = I.MWUI.templates.textNormal,
		name = "hexText",
		props = {
			position = util.vector2(45, 0),
			relativePosition = util.vector2(0.55, 0.05),
			anchor = util.vector2(0, -0.5),
			text = "Hex: ##" .. value:asHex():upper(),
		},
		content = ui.content{},
	}
	
	rgbText = {
		template = I.MWUI.templates.textNormal,
		name = "rgbText",
		props = {
			position = util.vector2(45, 15),
			relativePosition = util.vector2(0.55, 0.05),
			anchor = util.vector2(0, -0.5),
			text = string.format("R:%d G: %d B:%d", rr, gg, bb),
		},
		content = ui.content{},
	}
	
	----------------------------------------------------------------
	-- HEX / R / G / B EDIT LINES
	----------------------------------------------------------------
	hexInput = {
		template = I.MWUI.templates.textEditLine,
		name = "hexEditLine",
		props = {
			text = value:asHex(),
			size = util.vector2(65, 0),
			textAlignH = ui.ALIGNMENT.Center,
			textAlignV = ui.ALIGNMENT.Center,
		},
		content = ui.content{},
	}
	
	local hexEditLineBox = {
		template = I.MWUI.templates.box,
		name = "hexEditLineBox",
		props = {
			position = util.vector2(45, 60),
			relativePosition = util.vector2(0.55, 0.05),
		},
		content = ui.content{
			{
				template = I.MWUI.templates.padding,
				name = "hexEditLinePad",
				props = {},
				content = ui.content{ hexInput },
			},
		},
	}
	----------------------------------------------------------------
	-- BUILD R/G/B INPUT BOXES
	----------------------------------------------------------------
	local T_inputBoxes = {}
	
	for i, letter in ipairs { 'R', 'G', 'B' } do
		local inp, box = makeInputBox(i, letter, T_rgb[i])
		inputs[letter] = inp -- fill inputs registry
		T_inputBoxes[letter] = box
	end
	
	-- label boxes
	local labels = {
		R   = "R:",
		G   = "G:",
		B   = "B:",
		Hex = "Hex ##:",
	}
	
	local RLabelBox   = makeLabelBox("R",   labels.R)
	local GLabelBox   = makeLabelBox("G",   labels.G)
	local BLabelBox   = makeLabelBox("B",   labels.B)
	local HexLabelBox = makeLabelBox("Hex", labels.Hex)
	
	local labelsColumn = {
		type = ui.TYPE.Flex,
		name = "labelsColumn",
		props = {
			horizontal = false,
			align = ui.ALIGNMENT.Start,
			arrange = ui.ALIGNMENT.End,
		},
		content = ui.content{
			RLabelBox,
			GLabelBox,
			BLabelBox,
			HexLabelBox,
		},
	}
	
	local inputsColumn = {
		type = ui.TYPE.Flex,
		name = "inputsColumn",
		props = {
			horizontal = false,
			align = ui.ALIGNMENT.Start,
			arrange = ui.ALIGNMENT.Start,
		},
		content = ui.content{
			T_inputBoxes['R'],
			T_inputBoxes['G'],
			T_inputBoxes['B'],
			hexEditLineBox,
		},
	}
	
	local colorValueColumns = {
		type = ui.TYPE.Flex,
		name = "colorValueColumns",
		props = {
			horizontal = true,
			align = ui.ALIGNMENT.Start,
			arrange = ui.ALIGNMENT.Center,
			position = util.vector2(0, 0),
			relativePosition = util.vector2(0.55, 0.15),
		},
		content = ui.content{
			labelsColumn,
			I.MWUI.templates.interval,
			inputsColumn,
		},
	}
	
	----------------------------------------------------------------
	-- ATTACH EVENTS TO INPUT WIDGETS
	----------------------------------------------------------------
	hexInput.events = {
		textChanged = async:callback(onHexTextChanged),
		focusLoss   = async:callback(onHexFocusLoss),
	}
	
	----------------------------------------------------------------
	-- COLOR HISTORY PANEL
	----------------------------------------------------------------
	local history = loadColorHistoryForUi(historySection, presetColors)
	local historySlots = MAX_HISTORY_COLORS - presetSlotCount(presetColors)
	local historySwatchContent = {}
	local historySwatchContent2 = {}
	
	-- repaint after a delete
	local function refreshHistorySwatches()
		local hist = loadColorHistoryForUi(historySection, presetColors)
		for i = 1, MAX_HISTORY_COLORS do
			local swatch = historySwatches[i]
			if swatch then
				swatch.props.visible = hist[i] ~= nil
				swatch.props.color = hist[i] or util.color.rgb(0.2, 0.2, 0.2)
			end
		end
		if colorBoxElement then colorBoxElement:update() end
	end
	
	-- make a single swatch
	local function makeHistorySwatch(index, color)
		local swatchColor = color or util.color.rgb(0.2, 0.2, 0.2)
		local isVisible = color ~= nil
		
		local swatchImage = {
			type = ui.TYPE.Image,
			name = "historySwatchImage" .. index,
			props = {
				size = util.vector2(SWATCH_SIZE, SWATCH_SIZE),
				visible = isVisible,
				alpha = 1.0,
				inheritAlpha = false,
				resource = ui.texture{ path = 'white' },
				color = swatchColor,
			},
			content = ui.content{},
			userData = {
				colorIndex = index,
			},
			events = {
				mousePress = async:callback(function(coord, layout)
					local hist = loadColorHistoryForUi(historySection, presetColors)
					local idx = layout.userData.colorIndex
					if not hist[idx] then return end
					
					-- right click deletes, preset slots hold nothing
					if coord.button == 3 then
						if idx <= historySlots then
							removeColorFromHistory(historySection, hist[idx])
							refreshHistorySwatches()
						end
						return
					end
					
					loadColorFromHistory(hist[idx])
				end),
			},
		}
		
		historySwatches[index] = swatchImage
		
		local template = I.MWUI.templates.box
		if index > historySlots then
			template = I.MWUI.templates.padding
		end
		local swatchBox = {
			template = template,
			name = "historySwatchBox" .. index,
			props = {},
			content = ui.content{ swatchImage },
		}
		
		return swatchBox
	end
	
	for i = 1, math.ceil(MAX_HISTORY_COLORS/2) do
		local col = history[i]
		table.insert(historySwatchContent, makeHistorySwatch(i, col))
	end
	for i = math.ceil(MAX_HISTORY_COLORS/2)+1, MAX_HISTORY_COLORS do
		local col = history[i]
		table.insert(historySwatchContent2, makeHistorySwatch(i, col))
	end
	
	-- History label
	local historyLabel = {
		template = I.MWUI.templates.textNormal,
		name = "historyLabel",
		props = {
			text = "Recent Colors:  (right click removes)",
			textAlignH = ui.ALIGNMENT.Start,
			textAlignV = ui.ALIGNMENT.Center,
		},
		content = ui.content{},
	}
	
	-- Horizontal flex for swatches
	local historySwatchRow = {
		type = ui.TYPE.Flex,
		name = "historySwatchRow",
		props = {
			horizontal = true,
			align = ui.ALIGNMENT.Center,
			arrange = ui.ALIGNMENT.Start,
		},
		content = ui.content(historySwatchContent),
	}
	local historySwatchRow2 = {
		type = ui.TYPE.Flex,
		name = "historySwatchRow2",
		props = {
			horizontal = true,
			align = ui.ALIGNMENT.Center,
			arrange = ui.ALIGNMENT.Start,
		},
		content = ui.content(historySwatchContent2),
	}
	
	-- Container for the whole history section
	local historyPanel = {
		type = ui.TYPE.Flex,
		name = "historyPanel",
		props = {
			horizontal = false,
			align = ui.ALIGNMENT.Start,
			arrange = ui.ALIGNMENT.Center,
			position = util.vector2(0, 0),
			relativePosition = util.vector2(0.5, 0.71),
			anchor = util.vector2(0.5, 0),
		},
		content = ui.content{
			historyLabel,
			historySwatchRow,
			historySwatchRow2,
		},
	}
	
	----------------------------------------------------------------
	-- MAIN COLOR PICKER BOX
	----------------------------------------------------------------
	-- remembered position, centered on first open
	local screen = ui.layers[ui.layers.indexOf('Settings')].size
	local storedX, storedY = windowPosSection:get("x"), windowPosSection:get("y")
	local startPos
	if storedX and storedY then
		startPos = clampWindowPos(util.vector2(storedX, storedY))
	else
		startPos = util.vector2(util.round((screen.x - boxWidth) / 2), util.round((screen.y - boxHeight) / 2))
	end
	
	local colorPickerBox = {
		template = I.MWUI.templates.boxSolidThick,
		name = "colorPickerBox",
		props = {
			position = startPos,
		},
		userData = {
			isDragging = false,
			lastMousePos = util.vector2(0, 0),
		},
		events = {
			-- drag the whole window by any empty background, controls consume their own presses
			mousePress = async:callback(function(data, layout)
				if data.button == 1 then
					layout.userData.isDragging = true
					layout.userData.lastMousePos = data.position
				end
			end),
			mouseRelease = async:callback(function(_, layout)
				if layout.userData.isDragging then
					layout.userData.isDragging = false
					windowPosSection:set("x", layout.props.position.x)
					windowPosSection:set("y", layout.props.position.y)
				end
			end),
			mouseMove = async:callback(function(data, layout)
				if not layout.userData.isDragging then return end
				local delta = data.position - layout.userData.lastMousePos
				layout.userData.lastMousePos = data.position
				layout.props.position = clampWindowPos(layout.props.position + delta)
				if colorBoxElement then colorBoxElement:update() end
			end),
		},
		content = ui.content{
			{
				type = ui.TYPE.Widget,
				name = "sizeWrapper",
				props = {
					size = util.vector2(boxWidth, boxHeight),
				},
				content = ui.content{
					okBtn,
					cancelBtn,
					rsvMapImage,
					crossHair,
					scrollTrack,
					scrollButton,
					displayColorBox,
					hexText,
					rgbText,
					colorValueColumns,
					historyPanel,
				},
			},
		},
	}
	
	----------------------------------------------------------------
	-- ROOT LAYOUT
	----------------------------------------------------------------
	local combinedLayout = {
		name = 'MAIN',
		layer = "Settings",
		type = ui.TYPE.Text, -- Must be text widget to capture key events
		props = {
			size = displayArea,
			autoSize = false,
		},
		content = ui.content{ blocker, colorPickerBox },
		events = {
			keyPress = async:callback(function(e, layout)
				if e.code == input.KEY.Escape then
					set(initialColor)  -- Revert to initial color
					resetState(colorBoxElement)
					colorBoxElement = nil
					newColor = nil
					return
				end
			end),
		},
	}
	
	-- ensure UI is in sync with starting color
	newColor = value
	updateUIFromColor()
	
	return combinedLayout
end

----------------------------------------------------------------
-- PUBLIC ENTRY POINT
----------------------------------------------------------------
function M.open(set, value, argument)
	colorBoxElement = ui.create(makeColorPickerBox(value, set, argument))
end

function M.close()
	if colorBoxElement then
		colorBoxElement:destroy()
		colorBoxElement = nil
	end
end

-- ================================================================================= MENU =================================================================================

local colorPicker = M

local whiteTexture = ui.texture { path = 'white' }
local defaultArgument = {
	disabled = false,
}

local function applyDefaults(argument, defaults)
	if not argument then return defaults end
	if pairs(defaults) and pairs(argument) then
		local result = {}
		for k, v in pairs(defaults) do
			result[k] = v
		end
		for k, v in pairs(argument) do
			result[k] = v
		end
		return result
	end
	return argument
end

local function paddedBox(layout)
	return {
		template = I.MWUI.templates.box,
		content = ui.content {
			{
				template = I.MWUI.templates.padding,
				content = ui.content { layout },
			},
		}
	}
end

local function disable(disabled, layout)
	if disabled then
		return {
			template = I.MWUI.templates.disabled,
			content = ui.content {
				layout,
			},
		}
	else
		return layout
	end
end

I.Settings.registerRenderer(COLOR_RENDERER_ID, function(value, set, argument)
	argument = applyDefaults(argument, defaultArgument)
	
	local keyPressDetect = {
		type = ui.TYPE.Text,
		props = {
			autoSize = false,
			size = util.vector2(20, 20),
		},
		events = {
			keyPress = async:callback(function(e) if e.code == input.KEY.Escape then colorPicker.close() end end),
		},
	}
	local colorDisplay = {
		template = I.MWUI.templates.box,
		content = ui.content {
			{
				type = ui.TYPE.Image,
				props = {
					resource = whiteTexture,
					color = value,
					-- TODO: remove hardcoded size when possible
					size = util.vector2(20, 20),
				},
				content = ui.content{keyPressDetect},
			}
		},
		events = {
			mouseClick = async:callback(function()
				colorPicker.open(set, value, argument)
			end),
		},
	}
	local lastInput = nil
	local hexInput = paddedBox {
		template = I.MWUI.templates.textEditLine,
		props = {
			text = value:asHex(),
		},
		events = {
			textChanged = async:callback(function(text)
				lastInput = text
			end),
			focusLoss = async:callback(function()
				if not lastInput then return end
				if not pcall(function() set(util.color.hex(lastInput)) end)
				then
					set(value)
				end
			end),
		},
	}
	return disable(argument.disabled, {
		type = ui.TYPE.Flex,
		props = {
			horizontal = true,
			arrange = ui.ALIGNMENT.Center,
		},
		content = ui.content {
			colorDisplay,
			{ template = I.MWUI.templates.interval },
			hexInput,
		}
	})
end)

-- BONUS
-- only ONE menu script should handle this:
local mouseWheelBonusFunction = false
if not input.triggers["MenuMouseWheelUp"] then
	input.registerTrigger({
		key = "MenuMouseWheelUp",
		l10n = "none",
		name = "MenuMouseWheelUp",
		description = "",
	})
	input.registerTrigger({
		key = "MenuMouseWheelDown",
		l10n = "none",
		name = "MenuMouseWheelDown",
		description = "",
	})
	mouseWheelBonusFunction = true
end


local function onMouseWheel(direction)
	-- BONUS
	if mouseWheelBonusFunction then
		if direction >0 then
			if input.triggers["MenuMouseWheelUp"] then
				input.activateTrigger("MenuMouseWheelUp")
			end
		else
			if input.triggers["MenuMouseWheelDown"] then
				input.activateTrigger("MenuMouseWheelDown")
			end
		end
	end
	
	if not colorBoxElement or not scrollButton or not crossHair then
		return
	end
	
	local step = 5
	local currentY = scrollButton.props.position.y
	local newY = util.clamp(currentY - (direction * step), 0, 236)
	
	scrollButton.props.position = util.vector2(266, newY)
	
	local mx = crossHair.props.position.x
	local my = crossHair.props.position.y
	
	local h = util.round(util.remap(mx, 0, 256, 0, 360))
	local s = util.round(util.remap(my, 0, 256, 100, 0))
	local v = util.round(util.remap(newY, 0, 236, 100, 0))
	
	local r, g, b = hsvToRgb(h, s, v)
	newColor = util.color.rgb(r / 255, g / 255, b / 255)
	
	pickerSet(newColor)
	pickerUpdateUI()
end


return {
	engineHandlers = {
		onMouseWheel = onMouseWheel,
	}
}